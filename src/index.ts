...
export default class Db extends Queryable {
  ...
  private async getReplicaConnection () {
    for (let i = 0; i < this.replicaAttributes.length; i++) {
      try {
        this.replicas[i] ??= this.getPool(this.replicaAttributes[i], `Replica[${i}]`)
  ...
  async connect () {
    ...
    while (true) {
      try {
        this.pool ??= await this.getPool(this.poolAttributes, 'Main') // Do we want to continue doing this here or break this out?
